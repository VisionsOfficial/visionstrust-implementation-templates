        ...
        .then((data) => {
            ...
            `;
                ...
                if (datatypeIds[datatype.provenance] == undefined) {
                    ...
                    checkboxIds[datatype.provenance] = [
                        'checkbox' + datatype.id,
                    ];
                } else {
                    datatypeIds[datatype.provenance].push('datatype' + i);
                    checkboxIds[datatype.provenance].push(
                        'checkbox' + datatype.id
                    );
                }
            ...
            if (flow == 'import') {
                ...
                for (let i = 0; i < services.length; i++) {
                    ...
                    if (emailsExport)
                        hasEmail = emailsExport.find(
                            (item) => item.service === services[i]
                        );
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    <a class="dropdown-item" href="${
                        flow == 'import' ? data.tosImport : data.tosExport
                    }">${SERVICE_NAME}</a>
        `;
            ...
            document.getElementById(`${flow}PurposeDescription`).innerText =
                data.description;
            document.getElementById(`${flow}tbody`).innerHTML = html;
            document.getElementById(`${flow}Tos`).innerHTML = terms;

            if (flow == 'export') {
                let importLogo = document.getElementById(
                    'exportModalExportServiceLogo'
                );
                importLogo.src = data.serviceImportLogo;
                importLogo.alt = data.serviceImport;

                document.getElementById('noAccountServiceImport').innerText =
                    data.serviceImport;
            }
...
const startImportProcess = () => {
    ...
    for (let service of services) {
        ...
        let emailExport = document.getElementById(
            'emailExport-' + service
        ).value;
