const { SERVICE_KEY, SERVICE_SECRET_KEY, API_BASE_URL } = require('./config');
...
function consentExport(req, res) {
    ...
    if (req.body.signedConsent) {
        ...
        if (decryptedData) {
            ...
            request.post(
                {
                    url: API_BASE_URL + '/consents/exchange/token',
                    headers: {
                        Authorization:
                            'Bearer ' +
                            auth.getJwtToken(SERVICE_KEY, SERVICE_SECRET_KEY),
            ...
            res.status(404).json({
                ...
                message:
                    'Something went wrong when decrypting the signed consent.',
            });
        }
    }
}
